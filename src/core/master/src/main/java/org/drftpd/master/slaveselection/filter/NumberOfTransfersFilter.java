...
/**
 * Removes numberOfTransfers * multiplier from the score.
 *
 * @version $Id$
 */
public class NumberOfTransfersFilter extends Filter {
    protected float _multiplier;

    public NumberOfTransfersFilter(int i, Properties p) {
        super(i, p);
        ...
        for (Iterator<ScoreChart.SlaveScore> iter = scorechart.getSlaveScores().iterator(); iter.hasNext(); ) {
            ...
            switch (direction) {
                case Transfer.TRANSFER_RECEIVING_UPLOAD:
                    numberOfTransfers = status.getTransfersReceiving();
                    break;
                case Transfer.TRANSFER_SENDING_DOWNLOAD:
                    numberOfTransfers = status.getTransfersSending();
                    break;
            };

            score.addScore(-(long) (numberOfTransfers * _multiplier));
        }
    }
}
